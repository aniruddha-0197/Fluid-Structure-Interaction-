...
%% Nearest Neighbor Interpolation structure -> flow (completed)
HsfNN = zeros(Nf, Ns);
for i=1:Nf
    % IMPLEMENT HERE:
    ...
    r = zeros (Ns,1);
    for j=1:Ns
        r(j) = sqrt((Xs(j,1)-Xf(i,1))^2+(Xs(j,2)-Xf(i,2))^2);        
    end
    [r_min, Index] = min(r);
    HsfNN(i,Index) = 1; %Puts a value 1 in the HsfNN
    
end
 

%% Nearest Neighbor Interpolation flow -> structure (completed)
HfsNN = zeros(Ns, Nf);
for i=1:Ns
    % IMPLEMENT HERE:
    ...
    r = zeros (Nf,1);
    for j=1:Nf
        r(j) = sqrt((Xf(j,1)-Xs(i,1))^2+(Xf(j,2)-Xs(i,2))^2);        
    end
    [r_min, Index] = min(r);
    HfsNN(i,Index) = 1; %Puts a value 1 in the HfsNN
end


%% RBF interpolation structure -> flow (Completed)
PHI = zeros(Ns);
P   = zeros(Ns,3);
...
%% RBF interpolation flow -> structure (completed)
PHI = zeros(Nf);
P   = zeros(Nf,3);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% CHECKING FOR CONSISTENCY OF THE MATRICES %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i=1:Ns %Going through Ns row
    NN = round(sum(HfsNN(i,:))); %Checking row sum
    RBF = round(sum(HfsRBF(i,:))); %Checking row sum
    if NN ~= 1 %Evaluating 
        disp ('!!Nearest Number Matrix is NOT consistent (HfsNN)!!');
    end
    if RBF ~= 1 %Evaluating 
        disp ('!!Radial Basis Function Matrix is NOT consistent (HfsRBF)!!');
    end
end

for i=1:Nf %Going through Nf rows
    NN = round(sum(HsfNN(i,:)));  %Checking row sum
    RBF = round(sum(HsfRBF(i,:))); %Checking row sum
    if NN ~= 1 %Evaluating 
        disp ('!!Nearest Number Matrix is NOT consistent (HsfNN)!!');
    end
    if RBF ~= 1 %Evaluating 
        disp ('!!Radial Basis Function Matrix is NOT consistent (HsfRBF)!!');
    end
end
...
%% Show displacements
figure(1) %Displacements Graph
hold off
plot(Xi,Df_ex,'b-','LineWidth',2.0);
hold on
plot(Xf(:,1),Df_NN, 'r-', 'LineWidth',2.0);
plot(Xf(:,1),Df_RBF, 'g-', 'LineWidth',2.0);
legend('Exact','NN','RBF');
xlabel('x');
ylabel('displacement in fluid points');
set(gcf, 'PaperSize', [5.5 5.5]); %Set the paper to have width 5 and height 5.
...
figure(2)
hold off
plot(Xi,Pf_ex,'b-', 'LineWidth',2.0);
hold on
plot(Xs(:,1),Ps_NN, 'r-', 'LineWidth',2.0);
plot(Xs(:,1),Ps_RBF, 'g-', 'LineWidth',2.0);
xlabel('x');
...
% Df =  H * Ds
% One obtains: Ps = .... * Pf
Ps_NN_cv  = (Mf*HsfNN*Ms^(-1))'*Pf;
Ps_RBF_cv = (Mf*HsfRBF*Ms^(-1))'*Pf; 
Pf_ex     = sin(2*pi*Xi);

%% Show pressures
figure(2)
plot(Xs(:,1),Ps_NN_cv, 'r--', 'LineWidth',2.0);
plot(Xs(:,1),Ps_RBF_cv, 'g--', 'LineWidth',2.0);
